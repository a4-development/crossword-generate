...
exports.createCrosswordWords = void 0;
const crossword_1 = require("./lib/crossword");
const GENE_COUNT = 10;
function createCrosswordWords(baseWords) {
    const crossword = crossword_1.Crossword.initialize(baseWords);
    ...
    for (let i = 0; i < 100; i++) {
        ...
        do {
            let [gene1, gene2] = crossGene(geneWithEvaluations[0].gene, geneWithEvaluations[1].gene);
            if (Math.random() < 0.05) {
...
exports.createCrosswordWords = createCrosswordWords;
function initialGene(baseWords) {
    let genes = [];
...
function mutateGene(gene, count = 1) {
...
function crossGene(gene1, gene2) {
